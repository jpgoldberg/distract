/*
 * This file, density.c, is part of the distrat/askmap package
...
static float density(struct cm *, int);    
...
main(int ac, char **av)
{
    ...
    fprintf(stderr,
	"CM Density %s. (C) 1993-1995 by Jeff Goldberg\n", version);
	...
	if(map_A == CMNULL) {
	    fprintf(stderr, "density: problem with map\n");
	    exit(EINPUT);
	}
	fprintf(stdout, "Density(%s) = %.4f\n", map_A->name,
	    density(map_A, alpha));
...
static float
density(struct cm *m1, int alpha)
{
    unsigned char i, j;
    unsigned int denom, numer;
    extern int debug;

    denom = (m1->size * m1->size) - (alpha * m1->size);

    numer=0;
    for(i=1; i <= m1->size; ++i)
	for(j=1; j <= m1->size; ++j)
	    if(m1->table[i][j] != 0.0) ++numer;
